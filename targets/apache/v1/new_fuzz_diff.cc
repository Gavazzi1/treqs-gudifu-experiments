...
void* runServerThread(void*) {
  const char *my_argv[] = {"/tmp/httpd", "-f", "/src/httpd.conf", "-E", "/tmp/apache.log", "-X", NULL};
  const int my_argc = std::end(my_argv) - std::begin(my_argv) - 1;

  // This will run the ngx in a single process mode, and in our case in a single thread.
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t data_len) {
  ...
  writeToFile(response, "/logs/response_apache_"+request_sha_str, true);
  return 0;
}
