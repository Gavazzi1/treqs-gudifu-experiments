...
void* run_httpd_main_thread(void*) {
  const char *my_argv[] = {
    "/tmp/httpd",
    "-f",
    "/src/httpd.conf",
...
bool start_httpd(){
  ...
  pthread_create(&httpd_thread, NULL, &run_httpd_main_thread, NULL);
  sleep(1);
  return true;
}

extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t data_len) {
  static bool httpd_started = start_httpd();
  if(!httpd_started) {
    printf("err: httpd is not running!\n");
  }
