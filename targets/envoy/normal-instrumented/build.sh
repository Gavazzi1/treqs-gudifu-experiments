...
#bazel build --verbose_failures --dynamic_mode=off --spawn_strategy=standalone --genrule_strategy=standalone --local_cpu_resources=HOST_CPUS*0.256 --//source/extensions/wasm_runtime/v8:enabled=false --build_tag_filters=-no_asan --config=oss-fuzz --copt=-fsanitize=fuzzer-no-link //source/exe:envoy-static
bazel build --verbose_failures --dynamic_mode=off --spawn_strategy=standalone --genrule_strategy=standalone --local_cpu_resources=HOST_CPUS*0.256 --//source/extensions/wasm_runtime/v8:enabled=false --build_tag_filters=-no_asan --config=oss-fuzz //source/exe:envoy-static

find / -name envoy-static | grep "bin/source/exe/envoy-static$" | xargs -I file cp file /out
