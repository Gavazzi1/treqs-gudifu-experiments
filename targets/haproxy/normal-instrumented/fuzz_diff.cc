...
extern "C" int main2(int argc, char *const *argv);
...
void* run_haproxy_main_thread(void*) {
  const char *my_argv[] = {
    "/tmp/haproxy",
    "-f",
    "/tmp/haproxy.cfg",
  ...
  // thread.
  main2(my_argc, const_cast<char* const *>(my_argv));
  //main2(my_argc, (my_argv));
...
bool start_haproxy(){
  ...
  pthread_t haproxy_thread;
  pthread_create(&haproxy_thread, NULL, &run_haproxy_main_thread, NULL);
  sleep(1);
  return true;
}

extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t data_len) {
  static bool haproxy_started = start_haproxy();
  if(!haproxy_started) {
    printf("err: haproxy is not running!\n");
  }
