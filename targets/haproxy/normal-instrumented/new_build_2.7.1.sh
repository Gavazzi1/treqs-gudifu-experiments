...
sed 's/LDFLAGS = $(ARCH_FLAGS) -g$/LDFLAGS = $(ARCH_FLAGS) -g ${CFLAGS}/g' -i Makefile

make V=1 TARGET=generic CC="${CC} -fsanitize=fuzzer-no-link" ADDLIB="-lpthread /usr/lib/libFuzzingEngine.a"
